...
覆盖：换位（相邻互换/整块移动）、连续缺字计权、缺页标记、异体归并与冲突整批拒绝、
      ...
      "versions": [                         // 逐版差异
        {
          ...
          "diff": [
            ...
            { "type": "transposition", "pos": 1, "baseChars": ["地", "玄"], "chars": ["玄", "地"] }
          ]
        }
      ],
...
- **对齐**：动态规划求字级最小编辑距离，再合并为段级操作并按上表计权；
  同分候选按「对角 > 删除 > 插入」取先，保证结果确定。
- **缺页**：连续缺页列折叠为一个标记，对齐时可免费覆盖底本任意长度区段，
  差异类型为 `missing_page`、代价 0。已知限制：与缺页区紧邻的个别真缺字
  可能被并入缺页段（底本侧无法区分二者）。
- **换位**：启发式识别——相邻两字互换（`甲乙`→`乙甲`）与整块移动
  （缺失段与插入段字序列相同且相距 ≤ 8 字），均合并为一次 `transposition`。
